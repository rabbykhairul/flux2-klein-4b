...
# FLUX.2 [klein] 4B is Apache 2.0; the 9B is under BFL's non-commercial licence and is
# deliberately not an option here.
KLEIN_MODEL = os.environ.get("KLEIN_MODEL", "flux-2-klein-4b.safetensors")
# fp4 text encoder by default: 3.85 GB against 8.04 GB for bf16, and the encoder runs once
# per job rather than once per step, so the precision costs far less here than in the DiT.
KLEIN_TEXT_ENCODER = os.environ.get("KLEIN_TEXT_ENCODER", "qwen_3_4b_fp4_flux2.safetensors")
...
DEFAULT_WORKFLOW = {
    "10": {
        "inputs": {"unet_name": KLEIN_MODEL, "weight_dtype": "default"},
        "class_type": "UNETLoader",
        "_meta": {"title": "Load Diffusion Model"}
    },
    "11": {
        "inputs": {"clip_name": KLEIN_TEXT_ENCODER, "type": "flux2", "device": "default"},
    ...
    # nearest-exact and resolution_steps=1 both come from the official klein template.
    # FLUX.2 does not need the multiple-of-16 rounding the Qwen VAE does.
    "30": {
        "inputs": {
    ...
    # Output dimensions track the source image, not the garment.
    "40": {
        "inputs": {"image": ["30", 0]},
    ...
    "61": {
        "inputs": {"conditioning": ["60", 0]},
        "class_type": "ConditioningZeroOut",
        "_meta": {"title": "Negative (zeroed)"}
    },
    # Multi-reference works by chaining one ReferenceLatent per image through the
    # conditioning, positive and negative kept as parallel chains over the same latents.
    "70": {
        "inputs": {"conditioning": ["60", 0], "latent": ["50", 0]},
        "class_type": "ReferenceLatent",
        "_meta": {"title": "Ref+ Source"}
    },
    "71": {
        "inputs": {"conditioning": ["70", 0], "latent": ["51", 0]},
        "class_type": "ReferenceLatent",
        "_meta": {"title": "Ref+ Reference"}
    },
    "72": {
        "inputs": {"conditioning": ["61", 0], "latent": ["50", 0]},
        "class_type": "ReferenceLatent",
        "_meta": {"title": "Ref- Source"}
    },
    "73": {
        "inputs": {"conditioning": ["72", 0], "latent": ["51", 0]},
        "class_type": "ReferenceLatent",
        "_meta": {"title": "Ref- Reference"}
    },
    "80": {
    ...
    "90": {
        "inputs": {"samples": ["85", 0], "vae": ["12", 0]},
        "class_type": "VAEDecode",
        "_meta": {"title": "VAE Decode"}
    },
    "9": {
        "inputs": {"filename_prefix": "Flux2_Klein", "images": ["90", 0]},
...
def build_edit_workflow(
    ...
    """Build the klein edit graph.

    Signature matches the fp8 Qwen worker so the same callers and test payloads work
    against both. Four of its parameters have no analogue here and are accepted but
    ignored: `lora` (klein 4B distilled is natively 4-step, there is no adapter),
    `negative_prompt` (the negative branch is a ConditioningZeroOut of the positive,
    which is what the model was distilled against), and `shift`/`scheduler` (Flux2Scheduler
    derives its own sigmas from steps and resolution).
    """
    workflow = copy.deepcopy(DEFAULT_WORKFLOW)

    workflow["20"]["inputs"]["image"] = source_image
    workflow["21"]["inputs"]["image"] = reference_image if reference_image else source_image

    workflow["60"]["inputs"]["text"] = prompt
    workflow["82"]["inputs"]["noise_seed"] = seed if seed is not None else random.randint(0, 2**53)

    workflow["80"]["inputs"]["steps"] = steps
    # Distilled klein is trained for cfg=1; anything above it doubles the forward passes
    # for a model that has no unconditional branch to guide toward.
    workflow["83"]["inputs"]["cfg"] = cfg if cfg is not None else 1.0
    workflow["81"]["inputs"]["sampler_name"] = sampler

    if megapixels is not None:
        workflow["30"]["inputs"]["megapixels"] = megapixels
        workflow["31"]["inputs"]["megapixels"] = megapixels
...
QWEN_MODELS = {
    "diffusion_models/flux-2-klein-4b.safetensors": {
        ...
        "name": "FLUX.2 klein 4B (distilled)",
        "type": "unet"
    },
    "clip/qwen_3_4b_fp4_flux2.safetensors": {
        "url": "https://huggingface.co/Comfy-Org/flux2-klein/resolve/main/split_files/text_encoders/qwen_3_4b_fp4_flux2.safetensors",
        "relative_path": "clip",
        "filename": "qwen_3_4b_fp4_flux2.safetensors",
        "name": "Qwen3-4B text encoder (fp4)",
        "type": "clip"
    },
